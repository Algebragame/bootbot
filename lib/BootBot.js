...
class BootBot extends EventEmitter {
  constructor(options) {
	...
	if (options.log) {
	  ...
	  const logStream = require('fs').createWriteStream(options.log);
  ...
  start(port, ssl) {
    ...
    if (ssl !== undefined) {
      this.server = https.createServer(ssl, this.app).listen(this.app.get('port'));
    }
    else{
  ...
  sendMessage(recipientId, message, options) {
    ...
    if (!options || (options && options.typing === undefined) || (options && options.typing === true)) {
      if (!options) { options = {typing: true}; }
  ...
  getUserProfile(userId) {
    const url = `https://graph.facebook.com/${userId}?fields=first_name,last_name,profile_pic,locale,timezone,gender&access_token=${this.accessToken}`;
    return fetch(url)
      .then(res => res.json())
  ...
  say(recipientId, message, options) {
    if (typeof message === 'string') {
      ...
      if (message.quickReplies && message.quickReplies.length > 0) {
        ...
        return this.sendTextMessage(recipientId, message.text, [], options);
      }
    ...
    return quickReplies && quickReplies.map((reply) => {
      if (typeof reply === 'string') {
        ...
        if (reply.question_id) {
          let payloadTitle = reply.title;
          if (reply.explanation !== undefined) {
            ...
            delete reply.explanation;
          }
          const id = reply.question_id;
          const payload = `QID_${id}_ANS_${payloadTitle}`
          delete reply.question_id;
