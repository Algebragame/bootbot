...
class BootBot extends EventEmitter {
  ...
  chainSay(recipientId, messengeAry, options) {
    // https://stackoverflow.com/a/45429177/3719276
    const sequence(ary, fn) => {
      return arr.reduce(
    ...
    return sequence(messengeAry, say);
  }
  ...
  say(recipientId, message, options) {
    if (typeof message === 'string') {
      ...
      return chainSay(recipientId, message, options);
    }
      ...
      if (typeof reply === 'string') {
        ...
        if (reply.question_id) {
          ...
          return Object.assign({
            content_type: 'text',
            payload: `QID_${id}_ANS_${reply.title}`
          }, reply);
        }
        return Object.assign({
          content_type: 'text',
